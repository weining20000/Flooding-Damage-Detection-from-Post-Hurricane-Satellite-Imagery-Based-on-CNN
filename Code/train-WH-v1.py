...
train_loader = DataLoader(traindataFromFolders, batch_size=25, shuffle=True)
...
val_loader = DataLoader(valdataFromFolders, batch_size=25, shuffle=True)
...
model_vgg, history_vgg = train(model_vgg,
                       ...
                       n_epochs=10, # if change epoch number, don't forget to change the range limits in the visualization below
                       BATCH_SIZE = 4)
...
df_valid_loss = pd.DataFrame({'Epoch': range(0, 10), # Make sure the range is consistent with the Epoch number
...
test_loader = DataLoader(testdataFromFolders, shuffle=False)
